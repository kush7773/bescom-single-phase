 ...
 * =============================================================
 * v8.1: Fault-update bug fixes on top of v8. New in v8.1:
 *
 *  V8.1-1: Fixed double mutex release in mb_read_raw_try().
 *          When got>0 but no valid Modbus frame was found, the mutex was
 *          released twice, corrupting FreeRTOS mutex state and causing
 *          potential bus collisions on subsequent reads.
 *
 *  V8.1-2: modbus_detect_if_needed() now also reads and caches currents
 *          (s_cached_ri/yi/bi) every 2 seconds, not just voltages.
 *          Without this, overload faults (ROL/YOL/BOL/OL) never fired
 *          and no-load faults (RPNl/YPNl/BPNl) were permanently active
 *          because currents were stuck at 0.0f.
 *
 *  V8.1-3: publish_telemetry_now() now updates s_cached_ri/yi/bi alongside
 *          s_cached_rv/yv/bv so the fault check in the main loop always
 *          uses the freshest values.
 *
 * v9: NTP time sync + GPS-loss clock-freeze fix. New in v9 vs v7:
 *
 *  V9-1: NTP (SNTP) sync via WiFi — started immediately after WiFi connects.
 *        Uses pool.ntp.org + time.google.com as fallback.  NTP is the
 *        PRIMARY clock source; GPS sync is supplementary.  This means the
 *        relay works from first boot even if GPS never gets a fix.
 *
 *  V9-2: GPS clock-freeze bug fixed.  sync_time_from_gps() now tracks the
 *        last h/m/s/date it wrote and skips settimeofday() if GPS data has
 *        not changed since last call.  Without this, a lost GPS would cause
 *        settimeofday() to be called every 10 s with the frozen last-seen
 *        time, effectively stopping the system clock.
 *
 *  V9-3: clock_is_valid() replaces the s_gps.time_valid guard in
 *        update_latch_relay_solar().  Returns true when time(NULL) > any
 *        timestamp after 2023, regardless of GPS state.  This lets the
 *        relay operate on NTP time alone when GPS is unavailable.
 *
 *  V9-4: calc_sun_minutes() uses IST-shifted gmtime_r (not localtime_r) so
 *        tm_yday is correct even after 18:30 UTC (= midnight IST date rollover).
 *
 *  V9-5: gps_solar_update() rate-limits solar recalculation to once per 5 min
 *        to protect NVS flash endurance (was called every second from main loop).
 *
 * v7: Two targeted fixes on top of v6:
 *
 *  V7-1: Single relay click on startup.
 *        Removed the NVS re-assert from ccms_hw_init — on boot the relay
 *        fires exactly one RESET pulse (one click, lands OFF).  The solar
 *        window logic (update_latch_relay_solar) turns it ON at sunset.
 *
 *  V7-2: Electrical telemetry values formatted to 2 decimal places.
 *        All V/A/kW/kWh/kVA/kVAh/PF/freq fields are snprintf'd to "%.2f"
 *        and sent as JSON strings.  Non-electrical fields (lat, long,
 *        fault_code, no_lights_on) keep their native numeric types.
 *
 * v6: Three targeted fixes on top of v5:
 *
 *  V6-1: Latch relay NVS persistence + single reset click on startup.
 *        s_relay_on is saved to NVS on every relay_on()/relay_off() call.
 *        On boot, ccms_hw_init() fires exactly ONE reset pulse (relay_off)
 *        to guarantee a known OFF state — one click, no re-assert.
 *        The solar window logic turns it back ON if inside sunset/sunrise.
 *
 *  V6-2: Sunset/Sunrise latch relay control.
 *        update_latch_relay_solar() runs in the main loop whenever solar
 *        times are valid.  It turns the relay ON at sunset, OFF at sunrise,
 *        and — while inside the night window — re-checks every
 *        RELAY_RECHECK_INTERVAL_MS (5 min) that the latch is still asserted,
 *        re-pulsing if it has dropped.
 *
 *  V6-3: Telemetry aligned to 15-minute wall-clock grid.
 *        First publish fires immediately on startup (or on MQTT connect).
 *        Subsequent publishes are aligned to HH:00, HH:15, HH:30, HH:45
 *        regardless of boot time.  s_next_telemetry_epoch tracks the next
 *        grid slot in Unix seconds; it is recomputed after every publish.
 *
...
#define RELAY_RECHECK_INTERVAL_MS (5UL * 60UL * 1000UL) /* 5 minutes */
...
/* V6-3: next grid-aligned wall-clock epoch for telemetry publish.
 * 0 = "publish immediately on next loop iteration". */
static time_t s_next_telemetry_epoch = 0;

/* V9-1: NTP sync started flag — set once after WiFi connects */
static bool s_ntp_started = false;

/* V6-2: last time the latch relay was re-checked during the night window */
static uint64_t s_last_relay_recheck_us = 0;
...
static bool mb_read_raw_try(uint8_t slave_id, uint16_t reg_3xxxx,
  ...
  if (got > 0) {
    ...
    /* V8.1-1: mutex already released above — do NOT give again */
    ESP_LOGW(TAG,
...
static void modbus_detect_if_needed(void) {
  ...
  /* V8.1-2: Read voltages AND currents in one batch (regs 18..38 = 11 floats)
   * so that fault checks in the main loop have fresh current data too.
   * Previously only voltages were read here, leaving s_cached_ri/yi/bi at 0.
   * This caused overload faults to never fire and no-load faults to be
   * permanently active. */
  float b2[11] = {-1.0f, -1.0f, -1.0f, -1.0f, -1.0f, -1.0f,
                  -1.0f, -1.0f, -1.0f, -1.0f, -1.0f};
  bool read_ok = mb_read_multi_float(REG_VL1N, 11, b2) &&
                 b2[0] >= 0.0f && b2[1] >= 0.0f && b2[2] >= 0.0f;

  if (read_ok) {
    s_cached_rv = clamp_non_negative(b2[0]);  /* reg 18: R-phase V */
    s_cached_yv = clamp_non_negative(b2[1]);  /* reg 20: Y-phase V */
    s_cached_bv = clamp_non_negative(b2[2]);  /* reg 22: B-phase V */
    /* V8.1-2: Also cache currents (regs 34,36,38 = b2[8..10]) */
    s_cached_ri = clamp_non_negative(b2[8]);  /* reg 34: R-phase I */
    s_cached_yi = clamp_non_negative(b2[9]);  /* reg 36: Y-phase I */
    s_cached_bi = clamp_non_negative(b2[10]); /* reg 38: B-phase I */
    if (!s_modbus_online)
      ESP_LOGI(TAG, "Modbus back online V=%.1f/%.1f/%.1f I=%.2f/%.2f/%.2f",
               s_cached_rv, s_cached_yv, s_cached_bv,
...
static void relay_on(void) {
  ...
  s_relay_on = true;
  nvs_save_relay_state(true); /* V6-1: persist across reboots */
...
static void relay_off(void) {
  ...
  s_relay_on = false;
  nvs_save_relay_state(false); /* V6-1: persist across reboots */
...
static void sync_time_from_gps(void) {
  ...
  /* V9-2: Rate-limit to once per 10 s — prevents micro-backward jumps from
   * GPS parse latency variation (~±100 ms at 9600 baud). */
  static uint64_t s_last_gps_sync_us = 0;
  uint64_t now_us = esp_timer_get_time();
  if (s_last_gps_sync_us != 0 &&
      (now_us - s_last_gps_sync_us) < 10ULL * 1000ULL * 1000ULL)
    return;

  /* V9-2: Skip if GPS h/m/s/date hasn't changed since last call.
   * Without this guard, a lost GPS (no new NMEA sentences) causes
   * settimeofday() to be called every 10 s with the frozen last-seen
   * time, effectively stopping the system clock. */
  static int s_last_h = -1, s_last_m = -1, s_last_s = -1, s_last_d = -1;
  if (s_gps.hour   == s_last_h && s_gps.minute == s_last_m &&
      s_gps.second == s_last_s && s_gps.day    == s_last_d)
    return; /* GPS data frozen — leave ESP32 RTC to free-run */

  s_last_h = s_gps.hour;   s_last_m = s_gps.minute;
  s_last_s = s_gps.second; s_last_d = s_gps.day;
  s_last_gps_sync_us = now_us;
  ...
  ESP_LOGD(TAG, "GPS time sync: %04d-%02d-%02d %02d:%02d:%02d UTC",
           s_gps.year, s_gps.month, s_gps.day,
...
static int calc_sun_minutes(bool sunrise) {
  if (!s_gps.loc_valid)
    return -1;
  /* V9-4: ESP-IDF has no TZ set so localtime_r == gmtime_r (UTC).
   * Shift to IST manually so tm_yday is correct even after 18:30 UTC
   * (= midnight IST), which is when the date rolls over in India. */
  time_t now = time(NULL);
  time_t local_t = now + IST_GMT_OFFSET_SEC;
  struct tm t = {0};
  gmtime_r(&local_t, &t);
...
static void gps_solar_update(void) {
  ...
  sync_time_from_gps(); /* V9-2: stale-GPS protected */

  /* V9-5: Rate-limit solar recalc to once per 5 min.
   * Without this, solar_save_to_nvs() would be called every second,
   * wearing out the NVS flash in hours. */
  static uint64_t s_last_solar_us = 0;
  uint64_t now_us = esp_timer_get_time();
  if (s_last_solar_us != 0 &&
      (now_us - s_last_solar_us) < (5ULL * 60ULL * 1000ULL * 1000ULL))
    return;
  s_last_solar_us = now_us;
  ...
  if (sr >= 0 && ss >= 0) {
    ...
    solar_save_to_nvs();
    char srs[8], sss[8];
    format_hhmm(sr, srs, sizeof(srs));
    format_hhmm(ss, sss, sizeof(sss));
    ESP_LOGI(TAG, "Solar times: sunrise=%s sunset=%s (IST)", srs, sss);
  }
}
...
/* V6-1: NVS persistence for latch relay state.
 * Saved on every relay_on() / relay_off() so a reboot can re-assert
 * the correct state instead of leaving the latch in an unknown position. */
static void nvs_save_relay_state(bool on) {
  nvs_handle_t h;
  if (nvs_open(NVS_NAMESPACE, NVS_READWRITE, &h) != ESP_OK)
    return;
  nvs_set_u8(h, "relay_on", (uint8_t)on);
  nvs_commit(h);
  nvs_close(h);
}
...
static void check_and_publish_faults(float rv, float yv, float bv, float ri,
    ...
    // 1. Check Power Failure first (Meter might be offline)
    CHK(FIDX_ACPFl, "ACPFl", s_is_battery_mode);

    // Door Open: "push to open" switch. When door closes -> button pressed -> circuit opens -> reads 1 (pull-up).
    // When door opens -> button released -> circuit closes to GND -> reads 0.
    // Bypassing the 60s cooldown for immediate alerting.
    {
        bool _now = (gpio_get_level(DOOR_PIN) == 0);
        if (_now != s_fault[FIDX_DO].active) {
            s_fault[FIDX_DO].active = _now;
            s_fault[FIDX_DO].last_publish_us = now_us;
            publish_fault("DO", _now);
        }
    }

    // 2. Guard: If Modbus is offline, stop here
    if (!s_modbus_online || !s_cache_valid) {
        // REMOVED #undef CHK FROM HERE
        return; 
    }

    // 3. Check all other faults
    ...
    // 4. Finally undefine it at the very end
    #undef CHK
}
...
static void publish_telemetry_now(void) {
  ...
  float rv = 0, yv = 0, bv = 0, ri = 0, yi = 0, bi = 0;
  {
    ...
    if (mb_read_multi_float(REG_VL1N, 11, b2)) {
      ...
      s_cached_yv = yv;
      s_cached_bv = bv;
      /* V8.1-3: Also update current cache so fault checks use fresh data */
      s_cached_ri = ri;
      s_cached_yi = yi;
      s_cached_bi = bi;
    }
  ...
  /* --- Build payload --- */
  /* V7: Electrical values are formatted to 2 decimal places via snprintf
   * before being added as JSON strings.  Non-electrical fields (lat, long,
   * fault_code, no_lights_on) keep their native types. */
#define ADD_F2(obj, key, val)                                                  \
  do {                                                                         \
    char _buf[32];                                                             \
    snprintf(_buf, sizeof(_buf), "%.2f", (double)(val));                       \
    cJSON_AddStringToObject((obj), (key), _buf);                               \
  } while (0)
  ...
  ADD_F2(root, "current_a", avg_i);
  ADD_F2(root, "frequency", freq);
  ADD_F2(root, "pf", apf);
  ADD_F2(root, "kw", tkw);
  ADD_F2(root, "kwh", kwh);
  ADD_F2(root, "kva", tkva);
  ADD_F2(root, "kvah", kvah);

  /* R-phase — .2f */
  ADD_F2(root, "r_voltage", rv);
  ADD_F2(root, "r_current", ri);
  ADD_F2(root, "r_frequency", freq);
  ADD_F2(root, "r_pf", rpf);
  ADD_F2(root, "r_kw", rkw);
  ADD_F2(root, "r_kva", rkva);

  /* Y-phase — .2f */
  ADD_F2(root, "y_voltage", yv);
  ADD_F2(root, "y_current", yi);
  ADD_F2(root, "y_frequency", freq);
  ADD_F2(root, "y_pf", ypf);
  ADD_F2(root, "y_kw", ykw);
  ADD_F2(root, "y_kva", ykva);

  /* B-phase — .2f */
  ADD_F2(root, "b_voltage", bv);
  ADD_F2(root, "b_current", bi);
  ADD_F2(root, "b_frequency", freq);
  ADD_F2(root, "b_pf", bpf);
  ADD_F2(root, "b_kw", bkw);
  ADD_F2(root, "b_kva", bkva);
...
#undef ADD_F2
  ...
  if (payload) {
    int mid =
        esp_mqtt_client_publish(s_device_mqtt_client, topic, payload, 0, 1, 0);
    ESP_LOGI(TAG,
             "Telemetry published mid=%d | V=%.1f/%.1f/%.1f I=%.1f/%.1f/%.1f"
             " kWh=%.2f modbus=%s",
...
static void mqtt_device_handler(void *arg, esp_event_base_t base, int32_t eid,
  ...
  case MQTT_EVENT_CONNECTED:
    ...
    /* V6-3: reset epoch so the main loop fires an immediate publish */
    s_next_telemetry_epoch = 0;
    break;
...
static void ccms_hw_init(void) {
  ...
  /* V7: Single reset pulse on boot — guarantees latch is physically OFF.
   * No re-assert from NVS. The solar window logic (update_latch_relay_solar)
   * will turn it ON at sunset if we are inside the night window. */
  relay_off();
...
/* ========================================================================== */
/* V6-3: Grid-aligned telemetry scheduling                                   */
/* ========================================================================== */
/*
 * Returns the Unix epoch of the next 15-minute boundary AFTER 'now'.
 * Examples (IST = UTC+5:30):
 *   now = 05:01:XX  ->  next = 05:15:00
 *   now = 05:14:59  ->  next = 05:15:00
 *   now = 05:15:00  ->  next = 05:30:00  (boundary itself counts as "passed")
 *
 * We work in UTC seconds, which is fine — the grid is 15-min-aligned in any
 * timezone because 15 divides 60.
 */
static time_t next_telemetry_grid_epoch(void) {
  time_t now = time(NULL);
  /* Round up to the next multiple of TELEMETRY_INTERVAL_SEC */
  time_t rem = now % (time_t)TELEMETRY_INTERVAL_SEC;
  if (rem == 0)
    return now + (time_t)TELEMETRY_INTERVAL_SEC; /* exactly on boundary */
  return now + ((time_t)TELEMETRY_INTERVAL_SEC - rem);
}

/* ========================================================================== */
/* V6-2: Sunset/Sunrise latch relay automation                               */
/* ========================================================================== */
/*
 * Called from the main loop whenever s_solar_valid is true.
 *
 * Night window: sunset_min <= current_min_of_day  OR  current_min_of_day <
 * sunrise_min (handles the wrap around midnight correctly)
 *
 * Behaviour:
 *   - Entering night  (was day, now night): relay_on()
 *   - Entering day    (was night, now day): relay_off()
 *   - Mid-night check: if relay should be ON but s_relay_on==false, re-pulse.
 *     This covers the case where someone manually turned it off, or the latch
 *     lost its state due to a power glitch.
 */
/* Returns true when the current wall-clock time falls inside the night window */
static bool is_night_window(int current_min) {
  int sr = s_sunrise_min;
  int ss = s_sunset_min;

  /* Both values must be valid, and sunset must happen after sunrise in the day */
  if (sr < 0 || ss < 0 || ss <= sr)
    return false;

  /* Night window wraps midnight: cur_min >= sunset  OR  cur_min < sunrise */
  return (current_min >= ss || current_min < sr);
}

/* V9-3: Clock validity check — true when time(NULL) is past any date in 2024+.
 * Works with NTP or GPS as the time source. */
static bool clock_is_valid(void) {
  return time(NULL) > 1700000000UL; /* 2023-11-14 — safely post-epoch */
}

static void update_latch_relay_solar(void) {
  if (!s_solar_valid)
    return;
  /* V9-3: Guard against acting on a garbage clock (epoch 0).
   * In v7 this was s_gps.time_valid; now NTP can satisfy this check too. */
  if (!clock_is_valid())
    return;

  /* Get current IST time as minutes-of-day */
  time_t now_epoch = time(NULL);
  struct tm t = {0};
  time_t ist_epoch = now_epoch + IST_GMT_OFFSET_SEC;
  gmtime_r(&ist_epoch, &t);
  int cur_min = t.tm_hour * 60 + t.tm_min;

  bool night = is_night_window(cur_min);
  uint64_t now_us = esp_timer_get_time();

  if (night) {
    if (!s_relay_on) {
      /* Either just entered night window, or relay dropped — re-assert */
      ESP_LOGI(TAG,
               "Solar: night window (%02d:%02d) — relay ON"
               " (sunset=%02d:%02d sunrise=%02d:%02d)",
               cur_min / 60, cur_min % 60, s_sunset_min / 60, s_sunset_min % 60,
               s_sunrise_min / 60, s_sunrise_min % 60);
      relay_on();
      s_last_relay_recheck_us = now_us;
    } else {
      /* Already ON — periodic re-check to catch silent latch dropout */
      if ((now_us - s_last_relay_recheck_us) >=
          (RELAY_RECHECK_INTERVAL_MS * 1000ULL)) {
        s_last_relay_recheck_us = now_us;
        /* Re-pulse SET coil to guarantee latch is asserted */
        ESP_LOGD(TAG, "Solar: night recheck — re-asserting latch relay");
        gpio_set_level(RELAY_RESET_PIN, 1);
        gpio_set_level(RELAY_SET_PIN, 0);
        vTaskDelay(pdMS_TO_TICKS(250));
        gpio_set_level(RELAY_SET_PIN, 1);
        /* s_relay_on stays true; no NVS write needed — state unchanged */
      }
    }
  } else {
    /* Day window */
    if (s_relay_on) {
      ESP_LOGI(TAG,
               "Solar: day window (%02d:%02d) — relay OFF"
               " (sunrise=%02d:%02d)",
               cur_min / 60, cur_min % 60, s_sunrise_min / 60,
               s_sunrise_min % 60);
      relay_off();
    }
  }
...
void app_main(void) {
  ...
  /* V9-1: Start NTP sync immediately after WiFi connects.
   * SNTP_OPMODE_POLL re-syncs automatically every ~1 hour in the background.
   * The ESP32 RTC is updated by the SNTP stack via settimeofday() internally,
   * so time(NULL) becomes valid as soon as the first NTP response arrives
  ...
  if (!s_ntp_started) {
    ...
    esp_sntp_init();
    s_ntp_started = true;
    ESP_LOGI(TAG, "V9: NTP sync started (pool.ntp.org / time.google.com)");
    /* Wait up to 3 s for first NTP response so relay fires at correct solar
     * time without waiting for provisioning + MQTT (~2-3 min otherwise). */
    for (int _i = 0; _i < 6 && !clock_is_valid(); _i++)
      vTaskDelay(pdMS_TO_TICKS(500));
    if (clock_is_valid()) {
      update_latch_relay_solar();
      ESP_LOGI(TAG, "V9: early relay check done (NTP synced before provision)");
    }
  }
  ...
  while (1) {
    ...
    /* V6-3: Grid-aligned telemetry publish.
     * s_next_telemetry_epoch == 0 means "publish immediately" (set on MQTT
     * connect or first boot). Otherwise wait until wall-clock reaches the
     * next 15-min boundary (HH:00, HH:15, HH:30, HH:45 UTC, which aligns
     * to the same fixed boundaries in IST since 15 divides 60). */
    if (s_device_mqtt_connected) {
      time_t wall = time(NULL);
      if (s_next_telemetry_epoch == 0 || wall >= s_next_telemetry_epoch) {
        publish_telemetry_now();
        s_next_telemetry_epoch = next_telemetry_grid_epoch();
